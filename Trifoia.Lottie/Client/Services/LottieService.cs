...
{
    public class LottieService : ServiceBase, ILottieService, IService
    {
        public LottieService(HttpClient http, SiteState siteState) : base(http, siteState) { }

        private string Apiurl => CreateApiUrl("Lottie");

        public async Task<List<Models.Lottie>> GetLottiesAsync(int ModuleId)
        {
            List<Models.Lottie> Lotties = await GetJsonAsync<List<Models.Lottie>>(CreateAuthorizationPolicyUrl($"{Apiurl}?moduleid={ModuleId}", EntityNames.Module, ModuleId));
